 ...
 * 
 */
define('THUMBLIB_ABSPATH',$_SERVER['DOCUMENT_ROOT']);
/**
...
class Thumbnail {
     ...
     * @return Thumbnail
     */
    function Thumbnail($fileName, $fullpath = 0) {
        ...
        if ($fullpath) {
            $this->fileName             = THUMBLIB_ABSPATH.$fileName;
        } else {
            $this->fileName             = THUMBLIB_ABSPATH;
        }
    ...
    * @access private
    */
    function watermarkImageGD($file, $desfile, $wm_file, $position) {
		$imgobj = getimagesize($file);

        if ($imgobj['mime'] != "image/jpeg" && $imgobj['mime'] != "image/png" && $imgobj['mime'] != "image/gif") {
            return false;
        }
        if ($imgobj['mime'] == "image/gif" && !function_exists("imagecreatefromgif")) {
        	return false;
        }
        
        if ($imgobj['mime'] == "image/jpeg") {
            $image = @imagecreatefromjpeg($file);
        } else if ($imgobj['mime'] == "image/png") {
